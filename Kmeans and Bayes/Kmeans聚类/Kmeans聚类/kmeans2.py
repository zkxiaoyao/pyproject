...
def showCluster(dataSet, k, centroids, clusterAssment):
    ...
    for i in range(numSamples):
        ...
        ax1.scatter(dataSet[i, 0], dataSet[i, 1], dataSet[i, 2],c=cs[index])
    # 设置聚类中心的样式
    cs = ['b', 'c', 'g', 'k', 'm', 'r', 'w', 'y']
    # 绘制聚类中心
    for i in range(k):
        ...
        ax1.scatter(centroids[i, 0],centroids[i, 1], centroids[i, 2],c=cs[index2],s=200,marker="p")
...
def main():
    ...
    for line in fileIn.readlines():
        ...
        ass=str(a)
        dataSet.append([float(lineArr[0]), float(lineArr[1]),a])
        f.write("hjgjgjbj")
    # 聚类
